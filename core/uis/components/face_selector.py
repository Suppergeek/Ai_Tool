...
from core.common_helper import calc_float_step, calc_int_step
from core.face_analyser import get_many_faces
from core.face_selector import sort_and_filter_faces
from core.face_store import clear_reference_faces, clear_static_faces
from core.filesystem import is_image, is_video
from core.typing import FaceSelectorMode, FaceSelectorOrder, Gender, Race, VisionFrame
from core.uis.core import get_ui_component, get_ui_components, register_ui_component
from core.uis.typing import ComponentOptions
from core.uis.ui_helper import convert_str_none
from core.vision import get_video_frame, normalize_frame_color, read_static_image
...
def render() -> None:
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = core.choices.face_selector_modes,
	...
	with gradio.Group():
		with gradio.Row():
			FACE_SELECTOR_ORDER_DROPDOWN = gradio.Dropdown(
				...
				choices = core.choices.face_selector_orders,
			...
			FACE_SELECTOR_GENDER_DROPDOWN = gradio.Dropdown(
				...
				choices = [ 'none' ] + core.choices.face_selector_genders,
			...
			FACE_SELECTOR_RACE_DROPDOWN = gradio.Dropdown(
				...
				choices = ['none'] + core.choices.face_selector_races,
				value = state_manager.get_item('face_selector_race') or 'none'
			)
		with gradio.Row():
			face_selector_age_start = state_manager.get_item('face_selector_age_start') or core.choices.face_selector_age_range[0]
			face_selector_age_end = state_manager.get_item('face_selector_age_end') or core.choices.face_selector_age_range[-1]
			FACE_SELECTOR_AGE_RANGE_SLIDER = RangeSlider(
				label = wording.get('uis.face_selector_age_range_slider'),
				minimum = core.choices.face_selector_age_range[0],
				maximum = core.choices.face_selector_age_range[-1],
				value = (face_selector_age_start, face_selector_age_end),
				step = calc_int_step(core.choices.face_selector_age_range)
			)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = wording.get('uis.reference_face_distance_slider'),
		value = state_manager.get_item('reference_face_distance'),
		step = calc_float_step(core.choices.reference_face_distance_range),
		minimum = core.choices.reference_face_distance_range[0],
		maximum = core.choices.reference_face_distance_range[-1],
		visible = 'reference' in state_manager.get_item('face_selector_mode')
	)
