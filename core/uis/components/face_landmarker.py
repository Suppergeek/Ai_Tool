...
import core.choices
from core import face_landmarker, state_manager, wording
from core.common_helper import calc_float_step
from core.typing import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = core.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(core.choices.face_landmarker_score_range),
		minimum = core.choices.face_landmarker_score_range[0],
		maximum = core.choices.face_landmarker_score_range[-1]
	)
